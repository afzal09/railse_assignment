  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    switch (task.status) {
      case Status.notStarted:
        ...
        break;
      case Status.started:
        sideBarColor = AppColors.overdueBar; // Blue bar for started/active tasks
      ...
      case Status.completed:
        isCompleted = true;
        sideBarColor = AppColors.completedStatus; // No bar for completed tasks
        ...
        actionButton = null; // No action button for completed tasks
        isEditableDate = false;
        break;
      case Status.overdue: // Handled separately as it's a specific 'started' state visually
        sideBarColor = AppColors.overdueBar; // Blue bar for overdue tasks
    ...
    // Function to show the date picker for editing dates
          ...
          return Theme(
            data: ThemeData.light().copyWith(
              colorScheme: const ColorScheme.light(
                primary: AppColors.overdueBar, // Accent color for the date picker
                        ...
                        Row(
                          children: [
                            ...
                            IconButton(
                              ...
                              icon: const Icon(Icons.more_vert, size: 20, color: AppColors.greyColor),
                              padding: EdgeInsets.zero,
                        ...
                        if (task.priority == Priority.high)
                          Padding(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: AppColors.veryHighPriority.withOpacity(0.1),
                                borderRadius: BorderRadius.circular(4),
                              ),
                              child: Text(
                                'High Priority',
                                style: TextStyle(color: AppColors.veryHighPriority, fontSize: 11, fontWeight: FontWeight.bold),
                              ),
                            ),
                          ),
                      ],
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            color: isEditableDate ? AppColors.darkGray : AppColors.greyColor,
                            fontSize: 12,
                          ),
                        ),
                        if (isEditableDate)
                          const Icon(Icons.edit, size: 14, color: AppColors.greyColor),
                      ],
                    ),
                  ),
                  const SizedBox(height: 4),
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            color: (task.status == Status.notStarted) ? AppColors.darkGray : AppColors.greyColor,
                            fontSize: 12,
                          ),
                        ),
